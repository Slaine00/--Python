...
# 日付表示をオン・オフするボタンの作成
		self.b_date = tk.Button(self, text="Show Date", font=("",14), command=self.toggle_date)
		self.b_date.grid(row=1, column=0)
		
# 秒針の表示/非表示を切り替えるボタンの作成
		self.b_sec = tk.Button(self, text="Show Second Hand", font=("",14), command=self.toggle_sec)
		self.b_sec.grid(row=2, column=0)
...
# 日付表示ボタンが押されたときの call back

	def toggle_date(self):
		if self.show_date:
			self.b_date.configure(text="Show Date")
		else:
			self.b_date.configure(text="Hide Date")
		
		self.show_date = not self.show_date
		
# 秒針表示ボタンが押されたときの call back

	def toggle_sec(self):
		if self.show_sec:
			self.b_sec.configure(text="Show Second Hand")
		else:
			self.b_sec.configure(text="Hide Second Hand")
